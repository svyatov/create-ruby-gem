...
RSpec.describe 'create-gem integration' do
  ...
  it 'creates a gem from preset options end-to-end' do
    Dir.mktmpdir do |tmpdir|
      store = CreateGem::Config::Store.new(path: File.join(tmpdir, 'config', 'config.yml'))
      ...
      status = Dir.chdir(tmpdir) do
        CreateGem::CLI.start([gem_name, '--preset', preset_name], out: out, err: err, store: store)
      end
  ...
  it 'does not create a gem directory in dry-run mode' do
    Dir.mktmpdir do |tmpdir|
      ...
      status = Dir.chdir(tmpdir) do
        CreateGem::CLI.start([gem_name, '--preset', preset_name, '--dry-run'], out: out, err: err, store: store)
      end
