...
RSpec.describe CreateRubyGem::CLI do
  it 'lists presets' do
    out = StringIO.new
    store = instance_double(CreateRubyGem::Config::Store, preset_names: %w[fast full])
    detector = instance_double(CreateRubyGem::Detection::Runtime)
    runner = instance_double(CreateRubyGem::Runner)
    ...
    status = described_class.start(
      ...
      detector: detector,
      runner: runner,
      prompter: instance_double(CreateRubyGem::UI::Prompter)
    )
  ...
  it 'runs with preset in dry-run mode' do
    store = instance_double(CreateRubyGem::Config::Store)
    ...
    versions = CreateRubyGem::Detection::RuntimeInfo.new(
      ruby: Gem::Version.new(RUBY_VERSION),
      rubygems: Gem::Version.new(Gem::VERSION),
      bundler: Gem::Version.new('4.0.4')
    )
    detector = instance_double(CreateRubyGem::Detection::Runtime, detect!: versions)
    runner = instance_double(CreateRubyGem::Runner)
  ...
  it 'returns error when preset is missing' do
    err = StringIO.new
    store = instance_double(CreateRubyGem::Config::Store, preset: nil, last_used: {})
    versions = CreateRubyGem::Detection::RuntimeInfo.new(
      ruby: Gem::Version.new(RUBY_VERSION),
      rubygems: Gem::Version.new(Gem::VERSION),
      bundler: Gem::Version.new('4.0.4')
  ...
  it 'prints version' do
    ...
    expect(status).to eq(0)
    expect(out.string).to eq("#{CreateRubyGem::VERSION}\n")
  end
  ...
  it 'shows preset values' do
    out = StringIO.new
    store = instance_double(CreateRubyGem::Config::Store, preset: { 'ci' => 'github', 'test' => 'rspec' })
  ...
  it 'returns 130 on interrupt with user-friendly abort line' do
    store = instance_double(CreateRubyGem::Config::Store, last_used: {})
    runner = instance_double(CreateRubyGem::Runner)
    prompter = CLIFakePrompter.new
  ...
  it 'returns error for --show-preset with missing preset' do
    err = StringIO.new
    store = instance_double(CreateRubyGem::Config::Store, preset: nil)
  ...
  it 'doctor returns error on unsupported bundler version' do
    ...
    allow(detector).to receive(:detect!).and_raise(
      CreateRubyGem::UnsupportedBundlerVersionError, 'Unsupported bundler version: 1.0'
    )
